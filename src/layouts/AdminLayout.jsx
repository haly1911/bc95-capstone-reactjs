...
const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen flex bg-[#160d1d] relative overflow-x-hidden">
      ...
      <aside
        className={`
        fixed inset-y-0 left-0 z-50 w-54 shrink-0 bg-[#2a1b35] border-r border-[#442c54]/40 flex flex-col backdrop-blur-xl
        transition-transform duration-300 ease-in-out
        lg:static lg:translate-x-0 lg:bg-[#2a1b35]/40
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          ...
          <NavLink to="/admin/users-management" className={navLinkClassName} onClick={() => setIsSidebarOpen(false)}>
            <div className="text-[#F0BB3B]">
              <i className="fa-solid fa-users"></i>
            </div>
            <span>Người dùng</span>
          </NavLink>
          <NavLink to="/admin/movies-management" className={navLinkClassName} onClick={() => setIsSidebarOpen(false)}>
            <div className="text-[#F0BB3B]">
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full">
        <header className="bg-[#2a1b35]/20 border-b border-[#442c54]/30 px-4 sm:px-8 py-5 flex items-center justify-between shrink-0 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="lg:hidden text-white bg-[#342242]/60 border border-[#442c54]/40 p-2.5 rounded-xl hover:text-[#f0bb3b] transition-colors cursor-pointer"
            >
              <i className="fa-solid fa-bars text-lg"></i>
            </button>
            <h1 className="text-white font-bold text-sm sm:text-base tracking-wide flex items-center gap-2">
              <span className="hidden sm:inline-block h-2 w-2 rounded-full bg-[#f0bb3b] shadow-[0_0_8px_#f0bb3b] animate-pulse"></span>
              Hệ thống quản trị
            </h1>
          </div>
        ...
        <main className="flex-1 p-4 sm:p-8 overflow-auto bg-linear-to-b from-[#1c1125] to-[#110917]">
